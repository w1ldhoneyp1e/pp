...
{
public:
    static constexpr Money kInitialCash = 10'000;

    explicit Simulation(bool verbose);
    ~Simulation();

    void Run(bool parallel, std::chrono::seconds duration, const std::atomic<bool>& stopRequested);
...
private:
    ...
    void RunSingleThread(const std::chrono::steady_clock::time_point& deadline, const std::atomic<bool>& stopRequested);
    void RunParallel(const std::chrono::steady_clock::time_point& deadline, const std::atomic<bool>& stopRequested);
...
private:
    std::unique_ptr<EconomyContext> ctx_;
};
