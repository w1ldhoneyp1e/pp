...
struct PipelineResult
{
    ...
    void PrintTiming() const
    {
        ...
        std::cout << "Sequential section (archive generation): " << tarTime
                  << " s" << std::endl;
    }
};
...
struct Pipeline
{
    explicit Pipeline(const ParsedArgs &args) : args_(args) {}
    ...
    PipelineResult Run()
    {
        ...
        std::vector<std::string> gzPaths = BuildGzPaths(tempDir.Path(),
                                                        args_.inputFiles);
        ...
        bool compressionOk = args_.sequential
                                 ? runner.RunSequential(args_.inputFiles,
                                                        gzPaths)
                                 : runner.RunParallel(args_.inputFiles, gzPaths,
                                                     args_.numProcesses);
        if (!compressionOk)
        {
            tempDir.Remove(gzPaths);
