...
{
    return keyToIndex_[scancode];
}

PianoModel::Transition PianoModel::setKeyboardPressed(int keyIndex, bool pressed)
{
...
{
    if (mousePressedCount_[keyIndex] > 0)
    {
        --mousePressedCount_[keyIndex];
    }
...
{
    return logicalPressed_[keyIndex];
}
